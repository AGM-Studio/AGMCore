...
public class PlaytimeManager implements Listener {
    ...
    public static long getPlaytime(OfflinePlayer player, String unit) {
        ...
        if (MINUTE.contains(value1)) return PlaytimeManager.getPlaytime(player, TimeUnit.MINUTES) % 60;
        if (HOUR.contains(value1)) return PlaytimeManager.getPlaytime(player, TimeUnit.HOURS) % 24;
        if (DAYS.contains(value1)) return PlaytimeManager.getPlaytime(player, TimeUnit.DAYS);
        if (value1.startsWith("_TOTAL")) {
            String value2 = value1.substring(6);
            if (MINUTE.contains(value2)) return PlaytimeManager.getPlaytime(player, TimeUnit.MINUTES);
            if (HOUR.contains(value2)) return PlaytimeManager.getPlaytime(player, TimeUnit.HOURS);
            if (DAYS.contains(value2)) return PlaytimeManager.getPlaytime(player, TimeUnit.DAYS);
        }
        return -1;
    }
}
